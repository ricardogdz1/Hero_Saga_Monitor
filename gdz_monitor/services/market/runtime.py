"""
Fachada do mercado: logging, scraping e API Hero Saga com dependências já ligadas.

Compõe ``site_api`` (HTTP/parsing), ``domain`` (regras puras), ``formatters``
e ``adapters`` em funções prontas a usar pela camada de aplicação (``app/api.py``).
"""
...
from gdz_monitor.services.market import formatters
from gdz_monitor.services.market import site_api
from gdz_monitor.adapters import herosaga_api, persistence
...
from gdz_monitor.services.mvp_timer import mvp_catalog_matches_search

urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

try:
    from gdz_monitor.services.market.stores_scraper import (
        HerosagaScraper,
...
def _item_card_meta_from_details(details: dict) -> dict:
    return site_api.item_card_meta_from_details(details, item_card_keys=_ITEM_CARD_KEYS)
...
def _sync_iwork_name_from_sources(iwork: dict, history_data) -> None:
    site_api.sync_iwork_name_from_sources(
        iwork,
        history_data,
...
def group_sales_by_type(sales: list) -> dict:
    return domain.group_sales_by_type(sales)


def calculate_stats(sales: list) -> dict:
    return domain.calculate_stats(sales)


def item_emoji(name: str) -> str:
    return formatters.item_emoji(name)


def fmt_price(p) -> str:
    return formatters.fmt_price(p)


def fmt_price_stores(p) -> str:
    return formatters.fmt_price_stores(p)


def _alert_min_refinement(alert: dict):
    return domain.alert_min_refinement(alert)
...
def _format_home_min_prices_for_monitored(m: dict) -> str:
    return domain.format_home_min_prices_for_monitored(m, fmt_price_stores=fmt_price_stores)
...
def safe_get(d: dict, key: str, default: str = "N/A") -> str:
    return formatters.safe_get(d, key, default)


def clean_shop_name(name: str) -> str:
    return formatters.clean_shop_name(name)


def clean_json_response(text: str, content: bytes = None) -> str:
    return domain.clean_json_response(text, content, logger=logger)
